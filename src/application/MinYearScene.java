...
public class MinYearScene extends Pane {
	ArrayList<Integer> specificYearList=new ArrayList<>();
	ArrayList<Integer> minYearList=new ArrayList<>();

	public MinYearScene(Stage primaryStage,Scene oldScene) {
		...
		int count=1;	
		int minCount = specificYearList.get(0);
		...
		for(int i =0;i<minYearList.size();i++) {
			Label l1 = new Label("Year with minimum books: " + minYearList.get(i) + " : " + minCount + " books");
