...
public class SearchBookTitle extends Pane {

	public SearchBookTitle(Stage primaryStage,Scene oldScene) {
		...
		search.setOnAction(a ->{
			...
			for (int i=0;i<BookRecords.bookList.size();i++) {
				if(tf.getText().equalsIgnoreCase(BookRecords.bookList.get(i).getTitle())) {
