...
public class SearchAuthorByKeywordScene extends Pane {
	public SearchAuthorByKeywordScene(Stage primaryStage,Scene oldScene) {
		...
		search.setOnAction(a ->{
			...
			for(int i=0; i<BookRecords.bookList.size();i++) {
				if(BookRecords.bookList.get(i).getAuthor().toLowerCase().contains(keyword))
